...
?>
	<div class="col-md-10" style="background:#D5D8DC;min-height:570px;">
		...
		echo "</div>";
			...
			while($arr=mysqli_fetch_row($res))
			{
				...
				while($arr1=mysqli_fetch_row($res1))
				{
					...
					echo "<div class='col-md-2 game' style='height:120px;top:45px'>";
						echo "<div class='game-add'><button name='add' value='$arr[1]' class='btn btn2' type='submit'>UPDATE</button></div>";
					echo "</div></form>";
					echo "<div class='col-md-1 game' style='height:120px;top:45px'>";
						...
						echo "<div class='game-add'><button name='subject' value='$arr[1]' class='btn btn2'   type='submit' onClick='myFunction()'>Remove</button></div>";
			...
			echo "<div class='game-add'><a href='ordercfm.php?amountt=$sum'><button name='subject' value='$arr[1]' class='btn btn2' type='submit'>&#8377;Check out</button></a></div>";
			if(!isset($_SESSION['sadmin']))
			{
				echo "<form action='user/processOrder.php'>";
			}
				echo "<button name='subject' value='' class='btn btn2' type='submit'>Refresh
</button>";
